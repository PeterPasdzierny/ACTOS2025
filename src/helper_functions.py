...
def sort_raw_files_by_timestamp(raw_files):
...
def create_raw_subdir(flightdir):
...
def create_decoded_subdir(flightdir):
...
def write_raw_data(flightdir, stream_id, sensor_name, ptp, payload):
    outpath = f"{flightdir}{raw_subdir}{stream_id}_{sensor_name}.txt"
    with open(outpath, "a") as outfile:
        outfile.write(f"{ptp}*{str(payload)}\n")


def write_decoded_data(flightdir, raw_file, data):
